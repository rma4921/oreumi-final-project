...
then
  ...
  echo "## kill -15 $CURRENT_PID" >> /home/ec2-user/action/spring-deploy.log
  kill -15 "$CURRENT_PID"
  sleep 5
fi
...
nohup java -jar $DEPLOY_JAR \
  --spring.config.location=file:/home/ec2-user/config/application.properties \
  >> /home/ec2-user/action/spring-deploy.log 2>> /home/ec2-user/action/spring-deploy_err.log &
