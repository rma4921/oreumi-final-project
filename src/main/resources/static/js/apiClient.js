...
apiClient.interceptors.response.use(
    ...
    async error => {
        ...
        console.warn('[apiClient] 에러 발생:', error.config?.url, error.response?.status);
        ...
        if (error.response && error.response.status === 401 && !originalRequest._retry) {
            ...
            try {
                const refreshResponse = await axios.post('/api/token/refresh', {}, { withCredentials: true });
                console.info('[apiClient] 토큰 갱신 성공:', refreshResponse.status);

                return apiClient(originalRequest); // 원래 요청 다시 시도
            } catch (refreshError) {
