            ...
            li.innerHTML = `
                ...
                <div class="px-3 py-2 bg-white">
                    ...
                    <div class="d-flex align-items-center justify-content-end gap-2 text-muted" style="font-size: 0.85rem;">
                        <button class="btn btn-sm btn-outline-dark" onclick="editComment(${comment.id}, \`${comment.content.replace(
                /`/g, '\\`')}\`)"><i class="bi bi-pencil"></i></button>
...
window.submitEdit = async function (commentId) {
    const newContent = document.getElementById(
        `edit-input-${commentId}`).value.trim();
    try {
        const res = await fetch(`/api/comments/${commentId}`, {
            method: "PUT",
            headers: {"Content-Type": "application/json"},
