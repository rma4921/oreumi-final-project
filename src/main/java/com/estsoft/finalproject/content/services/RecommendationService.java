...
public class RecommendationService {
    ...
    public RecommendationService(AlanCommunicationService alanCommunicationService,
        StockPriceService stockPriceService, WebSearchService webSearchService) {
    ...
    public ResponseDto<String> getInvestmentTacticForCompany(String companyName) {
        ResponseDto<List<StockItem>> stockPriceData = stockPriceService.getStockPriceByName(
            companyName, 30);
        ResponseDto<List<NewsSearchItem>> newsItems = webSearchService.getSearchResults(companyName,
            5, false);

        StringBuilder stockPriceString = new StringBuilder();
        if (stockPriceData.getResponseCode() == HttpStatus.OK && stockPriceData.getSize() != 0) {
            stockPriceData.getItem().forEach(x -> {
                if (x.name().trim().equalsIgnoreCase(companyName)) {
                    stockPriceString.append(
                        " [ 일자: " + x.price() + " 주가: " + x.price() + " 전일대비 증감률: "
                            + x.fluctuationRate() + "% ] ");
                }
            });
        }
        ...
        if (newsItems.getResponseCode() == HttpStatus.OK && newsItems.getSize() != 0) {
            newsItems.getItem().forEach(x -> {
                newsItemsForCompany.append(
                    " [ 제목: <" + x.headline() + "> 날짜: <" + x.timestamp() + "> ] ");
            });
        }

        AlanResponseDto rez = alanCommunicationService.getResultFromAlan(
            SimpleAlanKoreanPromptBuilder.start().
                ...
                addContext("최근 해당 회사와 관련된 뉴스기사들은 다음과 같아: " + newsItemsForCompany.toString() + " ]")
                .buildPrompt());
        return ResponseDto.builder(rez.getContent())
            .message("The investment recommendation for the company is as follows.")
            .responseCode(rez.getResponseCode()).build();
    }
